...
resources=["/list","/wp-content","/wp-admin","/explore","/search/tag/list","/app/main/posts","/posts/posts/explore","/apps/cart.jsp?appID=","/rent"]
...
while (flag):
    ...
    if log_format == "CLF":
        message = '%s - %s [%s %s] "%s %s HTTP/1.0" %s %s\n' % (ip,user,dt,tz,vrb,uri,resp,byt)
    elif log_format == "ELF": 
