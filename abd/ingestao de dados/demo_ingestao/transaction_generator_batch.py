...
for DAY in range(1,11):
    ...
    out_list = []
    for x in range(N_REGISTERS):
        account_id = str(random.randint(1, 1000)).zfill(4)
        date_time = datetime.fromtimestamp(random.randint(min_ts, max_ts)).strftime('%Y-%m-%d %H:%M:%S')
    ...
    if 'db' in OUTPUT_TYPES:
        cursor.executemany("insert into mp_sales values (:1, :2, :3, :4, :5, :6, :7, :8, :9)", out_list)
        connection.commit()
        connection.close()